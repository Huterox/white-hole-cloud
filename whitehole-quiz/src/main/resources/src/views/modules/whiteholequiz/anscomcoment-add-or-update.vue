    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.comComentid) {
            this.$http({
              url: this.$http.adornUrl(`/whiteholequiz/anscomcoment/info/${this.dataForm.comComentid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.userid = data.ansComComent.userid
                this.dataForm.commentid = data.ansComComent.commentid
                this.dataForm.commentUserid = data.ansComComent.commentUserid
                this.dataForm.commentNickname = data.ansComComent.commentNickname
                this.dataForm.userNickname = data.ansComComent.userNickname
                this.dataForm.userimg = data.ansComComent.userimg
                this.dataForm.createTime = data.ansComComent.createTime
                this.dataForm.comment = data.ansComComent.comment
                this.dataForm.likeNumber = data.ansComComent.likeNumber
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/whiteholequiz/anscomcoment/${!this.dataForm.comComentid ? 'save' : 'update'}`),
              method: 'post',
