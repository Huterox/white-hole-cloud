    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.userid) {
            this.$http({
              url: this.$http.adornUrl(`/whiteholequiz/anscomcomentlike/info/${this.dataForm.userid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.userNickname = data.ansComComentLike.userNickname
                this.dataForm.creatTime = data.ansComComentLike.creatTime
                this.dataForm.commentid = data.ansComComentLike.commentid
                this.dataForm.commentUserid = data.ansComComentLike.commentUserid
                this.dataForm.commentNickname = data.ansComComentLike.commentNickname
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/whiteholequiz/anscomcomentlike/${!this.dataForm.userid ? 'save' : 'update'}`),
              method: 'post',
