    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.userid) {
            this.$http({
              url: this.$http.adornUrl(`/user/labelquizlike/info/${this.dataForm.userid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.label1 = data.labelQuizLike.label1
                this.dataForm.label2 = data.labelQuizLike.label2
                this.dataForm.label3 = data.labelQuizLike.label3
                this.dataForm.label4 = data.labelQuizLike.label4
                this.dataForm.label5 = data.labelQuizLike.label5
                this.dataForm.label6 = data.labelQuizLike.label6
                this.dataForm.label7 = data.labelQuizLike.label7
                this.dataForm.label8 = data.labelQuizLike.label8
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/user/labelquizlike/${!this.dataForm.userid ? 'save' : 'update'}`),
              method: 'post',
