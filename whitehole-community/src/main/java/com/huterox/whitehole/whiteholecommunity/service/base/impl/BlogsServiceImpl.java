...
public class BlogsServiceImpl extends ServiceImpl<BlogsDao, BlogsEntity> implements BlogsService {
    ...
    public PageUtils queryPage(Map<String, Object> params) throws Exception {
        ...
        IPage< BlogsEntity> page_params = new Query< BlogsEntity>().getPage(params);
        QueryWrapper< BlogsEntity> blogsEntityQueryWrapper = new QueryWrapper<>();
        /**
         * 社区的博文的一些管理不属于后台人员管理的部分！
         * */
        if(key!=null) {
            if (accurate == null) {
                blogsEntityQueryWrapper.like("communityid", key).or().
                        like("userid", key);
            } else {
                ...
                if (accurate.equals("single")) {
                    ...
                    if(table_name.equals("HoleNULL")){
                        blogsEntityQueryWrapper
                                .eq("status", status);
                    }else {

                        blogsEntityQueryWrapper.eq(table_name, key)
                                .eq("status", status);

                    }

                    if (order.equals("desc")) {
                        blogsEntityQueryWrapper.orderByDesc("communityid");
                    }

                } else if (accurate.equals("many")) {
                    Object accurate_query = params.get("accurate_query");
                    QueryWrapper< BlogsEntity> deserialize = (QueryWrapper< BlogsEntity>) SerializeUtil.deserialize(accurate_query.toString());
                    blogsEntityQueryWrapper = deserialize;
                }
            }
        }
