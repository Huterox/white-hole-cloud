<template>
  <el-dialog
    ...
    <el-form-item label="哪个博客的id，是在哪个博客下面的" prop="blogid">
      <el-input v-model="dataForm.blogid" placeholder="哪个博客的id，是在哪个博客下面的"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          blogid: [
            { required: true, message: '哪个博客的id，是在哪个博客下面的不能为空', trigger: 'blur' }
          ],
          createTime: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.userid) {
            this.$http({
              url: this.$http.adornUrl(`/whiteholeblog/commentlike/info/${this.dataForm.userid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.userNickname = data.commentLike.userNickname
                this.dataForm.commentid = data.commentLike.commentid
                this.dataForm.blogid = data.commentLike.blogid
                this.dataForm.createTime = data.commentLike.createTime
                this.dataForm.commentUserid = data.commentLike.commentUserid
                this.dataForm.commentUserNickname = data.commentLike.commentUserNickname
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/whiteholeblog/commentlike/${!this.dataForm.userid ? 'save' : 'update'}`),
              method: 'post',
